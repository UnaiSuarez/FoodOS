-- FoodOS — baseline inicial (reconciliación de historial, 2026-08-22)
-- Representación local exacta de la migración "initial_schema"
-- (20260629120159), la primera aplicada en remoto (rwxysqzurjsrevdhbejy).
-- Nunca había tenido archivo local: el proyecto empezó a versionar cada
-- cambio como migración individual DESPUÉS de este baseline, así que
-- `supabase migration list` la mostraba como "solo remoto" y `supabase db
-- push --dry-run` fallaba con LegacyDbPushMissingLocalError exigiendo esta
-- representación antes de poder calcular ningún diff.
--
